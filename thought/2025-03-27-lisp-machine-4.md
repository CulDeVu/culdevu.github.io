...
Making anything with rounded corners is painful. A minkowshi sum is just not a good primitive for that.
...
I've settled on this for the time being:

[image]

This probably isn't final.

# Boards

I haven't done much in this department. I put together the other 3 boards that I have the parts for.

Oh btw if you're wondering why all the pictures this time look like shit, it's because I recently got a Fairphone 5. I took all the pictures with it, not realizing my 5 year old Pixel 3A takes less blurry pictures. The phone as that huge iPhone-like multi-camera area on the back, but rest assured, no award-winning movies are going to be shot on the FP5.

img(rack.jpeg)[All 5 boards accounted for.]

img(open_circuit.jpeg)[I learned that the hotplate isn't some magical device that make mistakes go away.]
...
Not much here either. I wrote a simple bare-bones lisp compiler. Full source below.

I still haven't gotten around to writing a `letrec*` for the outer compiler, so still doing that manually.

Since I don't have any keyboard input yet, I just hardcode the ASCII keycodes: `40 43 32 40 43 32 50 53 32 53 41 32 56 41` which corresponds to the program `(+ (+ 25 5) 8)`. It then parses and compiles it. By "compiles" I mean building the lists in-memory, translating the `"+"` string to a literal `+` builtin function, etc.
...
What I'm trying to say is that a program of this size is likely to be the limit of what the Arduino simulator will be capable of.

Note that I plan on running programs in RAM in the final build. On reset, the first microinstructions will copy the program from ROM to RAM. Also in the finished build I have enough address lines for 16K cons cells, meaning it will be able to run programs about 40x as large.

When it first ran on the hardware, it took around 5m 40s to execute. Very bad.
