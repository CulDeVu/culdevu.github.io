...
In the early 2000s Alan Kay founded [todo: did he? with a co-creator?] the Viewpoints Research Institute, or VPRI. It's a typical Alan Kay endevor: reinvent computing in the image of whatever has been on his mind recently. This time it's source code size. In my own words, I'd say the problem statement is "can we make a computing system that has 95% of the functionality, 95% of the ease of use, but 1/10000th of the lines of code of systems today?", where "today" meant 2005. He often mentioned a target of 20kLOC for the whole computing stack.
...
The main vehicle by which they worked towards their goal is a program they created called Frank. Frank is one of those document-centric computing models. Documents could embed text, images, programs, etc. I don't think the source code to Frank was ever released.

After 2012, the VPRI slowed down significantly. Their website lists 2018 as the official end of the institute, but Alan Kay is listed as an author to only 2 papers after 2013, down from multiple papers a year. [todo: check all of the historical context papers dates and make sure.] [todo: is this really a good evidence of my claim?] [todo: something like "I guess they felt like they accomplished their goal?"] Sometime around June 2025 the SSL certs on their site broke. [todo: it's actually okay on my phone. Investigate?] [maybe include "The STEPS project ended in 2012. The subsequent efforts (CDG, and later, HARC) did not last." from https://github.com/damelang/nile/issues/3 ?]

2. Gezira

In all of [todo: all of? or just most of?] the annual NSF progress reports, a big deal is made about the rendering system. That's what I want to talk about.

The goal was to come up with a way to do all of the 2D graphics necessary to render text and shapes and such in as few lines of code as possible. They would do this by creating a programming language that could efficiently describe how to render.
...
Here is the "rendering formula". For a pixel with opposite corners (x, y) and (x+1, y+1), the "coverage" of a single line segment AB is:
...
This is how it is written on all of Gezira's materials. As far as I can tell, the formula is always presented as-is, and is never explained.

Personally, I think this form is unecessarily opaque. Here's my version:

$$
\begin{aligned}
\text{trapezoid}(P,Q) = & (Q_y - P_y)(x + 1 - \frac{Q_x + P_x}{2}) \\
\text{snap}(P) = & \min(x+1, \max(x, P_x)), \min(y+1, \max(y, P_y)) \\
\text{clip}(P) = & \text{snap}(\frac{1}{m} (\text{snap}(P)_y - P_y) + P_x, m (\text{snap}(P)_x - P_x) + P_y) \\
coverage(AB) = & \text{trapezoid}(\text{snap}(A), \text{clip}(A)) + \\
& \text{trapezoid}(\text{clip}(A), \text{clip}(B)) + \\
& \text{trapezoid}(\text{clip}(B), \text{snap}(B))
\end{aligned}
$$

imgcmp(rendering_formula_simple.svg)[](rendering_formula.svg)[]

`trapazoid(P,Q)` computes the area of a trapazoid formed by P, Q, and the projections of P and Q on the *right edge* of the pixel. `snap(P)` and `clip(P)` are used to compute which portion of the line is inside the pixel, if any. These are then put together to compute the area of the intersection of the pixel square and the line, if the line were *swept right*.

The area is signed as well, thanks to the $(Q_y - P_y)$ term in trapazoid. So if we were computing the total coverage of a closed polygon, any extra coverage for a line segment will get undone by a later line segment that closes the loop.

The total coverage has that extra $\min( | ... | , 1)$ term, presumably to not enforce a particular winding direction, and to handle multiple overlapping shapes.

This should remind you of other polygon area and winding rule algorithms. Pretty standard technique. It naturally supports the even-odd winding rule.
...
[image of gezira aliasing on a window shutter pattern]

Why does `trapazoid(P,Q)` sweep the area out to the right instead of left? I won't get into it now, but it's because Gezira renders scanlines from left to right. If you sort the line segments you can render an entire scanline by updating an accumulator.

It's important to note that the gezira source code does NOT use this rendering formula as described. For reasons we'll get into later, gezira actually pre-clips line segments. The line segments passed into the coverage computation code are already entirely contained in their given pixel. So there's no need for `clip()` and `snap()`.

The rendering formula that's actually being used looks like:

$$
\text{coverage}(AB) = (B_y - A_y)(x + 1 - \frac{Bx + Ax}{2})
$$

So if the rendering formula is supposed to be the basis of all rendering that powers this computing system, how does that work? There's more you might want to render than filled shapes. How does stroking work? Or texturing?
...
Luckily, the output from some of the build stages are checked in to the repo, so we can still observe a lot. For example, you can see the output .c files.

Despite the fact that there's an "official" repo, there seem to be lots of versions of the source out there. It's like every time I see some source listing from this project there's some variation in syntax or data type name or something. Very odd.
...
This isn't even going into the several full reimplementations in other languages.

We're going to be looking at the Gezira source shortly. I'm torn between which version to show. The "official" source is the most obvious one. When someone wants to see the source code to Gezira they'd go to the gezira repo and look under `nl/`. 

However, I think I like the js demo code the most for its stylistic choices. From the file modify dates it also seems like the latest edition. And most importantly, of all of the Gezira versions, only this one *actually runs!*

Unfortunately there are some stylistic issues. In the "official" source operators get applied elementwise over vectors implicitly, which is nice. I guess they never implemented that in the js demo.
...
If we're not building it, what are we doing? Well, the main way I think anyone understands Nile and Gezira these days is from link(https://tinlizzie.org/dbjr/high_contrast.html)[Bret Victor's demo site].

Yup! The famous Bret Victor made a little demo site for Gezira in 2012, explorable-explanations style. You can mouse over pixels, beziers, samples, and see how the data flows through the Gezira pipeline. You can click the stages to expand and see the sub-stages! Next to each stage is that stage's source code, and as you hover over the elements you see e.g. the branches that element took in the source. Very cool!
...
The site does have one small bug, pretty inconsequential bug: `PadGradient` should be `Real >> Real` and not `Point >> Point`.

It's important to mention that the demo site is NOT interpretting Gezira code. The source listings on the side *are* Gezira source code, but the interactive stuff is rewritten by hand in javascript. So a syntax error like this doesn't affect the demo.

Also, the way that highlighting works on the site can be confusing. The way that `CombineEdgeSamples` works makes it seem like there's an off-by-one error, even though there isn't.

What I want to do for the rest of this post is do a breakdown of the Gezira rendering pipeline, specifically the part shown on the demo site. I'll be going through each stage, explaining the code, and giving my thoughts and opinions.
...
Gezira actually operates on beziers, which it then deconstructs into line segments. Surprisingly, it uses *quadratic* beziers instead of cubic.
...
The first major stage of rasterization takes in a list of quadratic beziers and output `CoverageSpan`s.
...
The eagle-eyed might notice two different operators here: `⇒` and `→`. In my editor they look *very* similar. No idea what the difference between them are.

The other differences aren't important. In the "official" source `DecomposeBeziers` and `CombineEdgeSamples` are streams, whereas in the new source they're functions that return streams.

Like I've mentioned already, the source listed on the demo site actually comes from Nile js compiler demo. So the source listed here isn't going to quite match up exactly with the demo site.

One really interesting difference is the lack of distinction between these two operators. Again, no idea.

2.1 DecomposeBeziers
...
The formula they go for here is:
...
This is a nice demonstration of the STEPS conceit. `floor` and `ceil` are very common and useful in graphics, so they're given their own operators. In nile, these operators are applied elementwise.

We also see the and operator, `∧`, being applied once elementwise, and then again across elements.
...
And actually, min and max associate (but don't commute) with each other. So statements like `min ▷ A ◁ max` make sense. This is used to clamp numbers in a couple places in gezira. Very cool!
...
This isn't used because it has a small bug:
...
This has a type error, it should be SpanCoverage, not EdgeSpan. Again, not present in the full source code.

...

3.2 Gradient

All "textures" in Gezira are gradients. Linear and radial are your two options.
...
That being said there are also some functions that manipulate `ColorSpan`s. Those aren't used in the demo and I don't know how they're supposed to be used.

In the demo, this is done in the most obvious way. It pipes each `SpanCoverage` through a couple functions to choose a color for that point, and then constructs a pixel.

Actually, it does a `DupZip`, which splits the stream into a Point [todo:]
...
Unlike the previous functions, this one starts showing off how functions work in Nile. It looks functional, like `ProjectLinearGradient` is a function taking two points, whose return value is a function `PointCoverage >> Real`. But that's not quite right. It returns an object that transforms *streams*, like with `CombineEdgeSamples` above. [todo: re-explain this]

[todo: something like "presumably this does PointCoverage >> Real and not PointCoverage >> PointCoverageWithGradient and just skip the DupZip step because you might want to gradient between compositors?]

In the demo source, this is typed as `PointCoverage >> Real`, presumably make the pipeline easier.
...
This one is split off from `LinearGradient` because, in the full source, there are 2 other gradient manipulation functions: `RepeatGradient` and `ReflectGradient`. `PadGradient` is meant to be useful for both `LinearGradient` and `RadialGradient`. 
...
This one is short. I haven't looked at this too closely and don't have much to say.
